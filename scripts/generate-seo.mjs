...
const hubLinks = item => searchHubs.filter(h=>h.items.some(i=>i.id===item.id));
function hubHtml(hub,page){
 ...
 const description=`${hub.label} ${hub.items.length}곳의 등록 주소와 장소 설명을 비교하세요. 동반 조건은 방문 전 확인하고 발자국 지도에서 주변 장소를 찾아보세요.`;
 ...
 <h1>${escapeHtml(hub.label)}</h1><p>${escapeHtml(description)}</p><div class="note"><strong>방문 전에 확인하세요</strong><p>반려견 동반 가능 공간, 크기·마릿수 제한, 이동장 사용 여부와 추가 요금을 장소에 직접 문의하세요. 등록 정보는 현장 운영과 다를 수 있습니다.</p></div>
 <p>전체 ${hub.items.length}곳 · ${page}/${total}페이지</p><ul>${items.map(item=>`<li><h2><a href="${detailPath(item,hub.type)}">${escapeHtml(item.name)}</a></h2><p>${escapeHtml(item.category)}</p><p>${escapeHtml(item.address || '주소 미등록')}</p><p>${escapeHtml(compact(item.description).slice(0,180))}</p></li>`).join('')}</ul>
...
function detailHtml(item, type, url) {
  ...
  const hubs = type.table === 'facilities' ? hubLinks(item) : [];
